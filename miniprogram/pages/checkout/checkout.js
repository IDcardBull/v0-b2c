...
Page({
  ...
  onLoad() {
    ...
    this.setData({ statusBarHeight: app.globalData.statusBarHeight, navBarHeight: app.globalData.navBarHeight, items, remark, total })
    this.loadAddress()
  },
  async loadAddress() {
    try {
  ...
  async submit() {
    ...
    const items = this.data.items.map((item) => ({ skuId: item.skuId, qty: item.qty }))
    if (!items.length || items.some((item) => !item.skuId)) {
      wx.showToast({ title: '器物信息不完整', icon: 'none' })
      return
    }
    this.setData({ submitting: true })
    try {
      const order = await api.order.create({ items, addressId: this.data.address.id, remark: this.data.remark })
  ...
  async pay(orderId) {
    try {
      ...
      wx.showModal({ title: '支付暂不可用', content: err.message || '请联系管理员处理支付配置', showCancel: false })
